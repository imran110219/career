...
Use `scripts/build_master_resume.py` to regenerate the master PDF source from `master/resume.md`.

```bash
python3 scripts/build_master_resume.py
textutil -convert docx -output master/resume.docx /tmp/master_resume.rtf
```

Notes:

- `master/resume.pdf` is generated by the script.
- `master/resume.docx` is generated from `/tmp/master_resume.rtf` using `textutil`.
